...
{
    public class ChatFeatureChecker : AbpTemplateCoreServiceBase, IChatFeatureChecker
    {
        ...
        private void CheckChatFeaturesInternal(int? sourceTenantId, int? targetTenantId, ChatSide side)
        {
            ...
            {
                if (!_featureChecker.IsEnabled(sourceTenantId.Value, AppFeatureNames.ChatFeature))
                {
                ...
                if (targetTenantId.HasValue)
                {
                    ...
                    if (!_featureChecker.IsEnabled(sourceTenantId.Value, AppFeatureNames.TenantToTenantChatFeature))
                    {
                ...
                {
                    if (!_featureChecker.IsEnabled(sourceTenantId.Value, AppFeatureNames.TenantToHostChatFeature))
                    {
            ...
            else
            {
                if (targetTenantId.HasValue)
                {
                    if (!_featureChecker.IsEnabled(targetTenantId.Value, AppFeatureNames.TenantToHostChatFeature))
                    {
