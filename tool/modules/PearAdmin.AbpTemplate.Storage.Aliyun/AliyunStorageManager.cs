...
namespace PearAdmin.AbpTemplate.Storage.Aliyun
{
    public class AliyunStorageManager : IStorageManager
    {
        ...
        public AliyunStorageManager(Configuration configuration)
        {
            _configuration = configuration;
            _ossClient = AliyunClientFactory.Create(_aliyunConfig);
        }

        public Task GetBucketListAsync()
        {
            var bucketList = _ossClient.ListBuckets();
            return Task.FromResult(bucketList);
        }

        public Task UploadFile(string objectName, string filePath, string contentType, string bucketName = "default", string location = "us-east-1")
        {
        ...
        public Task<string> GetFileUrlAsync(string bucketName, string objectName, int expiresInt = 604800)
        {
            throw new NotImplementedException();
        }
        ...
        //            _ossClient.PutObject(_aliyunConfig.BucketName, key, source, objectMeta).HandlerError("上传对象出错");
        //        });
        //    }
        //    catch (Exception ex)
        //    {
        ...
        ///// <param name="blobName"></param>
        ///// <returns></returns>
        //public async Task<string> GetBlobUrl(string containerName, string blobName) => await Task.Run(() => $"{_aliyunConfig.BaseUrl}/{containerName}/{blobName}");
